...
import com.study.jpastudy.chap06_querydsl.dto.GroupAverageAgeDto;
...
class QueryDslGroupingTest {
    ...
    @Test
    @DisplayName("성별별로 그룹화하여 아이돌의 숫자가 3명이하인 그룹만 조회")
    void groupByGenderTest() {
        ...
        for (Tuple tuple : idolList) {
            System.out.println("tuple = " + tuple.get(idol.group));
        }
        System.out.println("\n\n\n");
    }

    @Test
    @DisplayName("연령대별로 그룹화 하여 아이돌 수를 조회")
    void ageGroupTest() {
        /*
            SELECT
    ...
    @Test
    @DisplayName("그룹별로 그룹명과 평균나이를 조회한다. 평균나이가 20세와 25세 사이인 그룹만 조회한다")
    void searchGroupAgeAvg() {
        //given
        List<Tuple> fetch = factory.select(idol.group.groupName, idol.age.avg())
                .from(idol)
